...
private:
	BSTreeNode *root;
	BSTreeNode* FindKthNode(BSTreeNode* root, int &k);//return node with the k-th key
...
public:
	...
	void DeleteRoot();
	void Insert(int id, const char* name, int &numComp);
	...
	Person* FindKthPerson(int &k, int &numComp);//return k-th person by ID
	void PrintTree();
};
